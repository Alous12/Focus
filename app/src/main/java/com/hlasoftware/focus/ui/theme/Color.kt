package com.hlasoftware.focus.ui.theme

import androidx.compose.ui.graphics.Color

// Custom App Colors based on the new design
val Night = Color(0xFF121212)
val EerieBlack = Color(0xFF1E1E1E)
val Jet = Color(0xFF373737)
val Gray = Color(0xFF777777)
val MidnightGreen = Color(0xFF0E3C4C)
val MidnightGreenLight = Color(0xFF5A8A9B)
val OrangeAccent = Color(0xFFF9A825) // Color for the Floating Action Button

val IndicatorClass = Color(0xFFE57373) // Light Red
val IndicatorTask = Color(0xFFDCE775) // Lime Green
val IndicatorMeeting = Color(0xFF64B5F6) // Light Blue


// Original Default Colors
val Purple80 = Color(0xFFD0BCFF)
val PurpleGrey80 = Color(0xFFCCC2DC)
val Pink80 = Color(0xFFEFB8C8)

val Purple40 = Color(0xFF6650a4)
val PurpleGrey40 = Color(0xFF625b71)
val Pink40 = Color(0xFF7D5260)

val AuthBackground = Color(0xFF1C1C1E) // Fondo oscuro general
val AuthArrowColor = Color(0xFF9C27B0) // Púrpura/Magenta para la flecha
